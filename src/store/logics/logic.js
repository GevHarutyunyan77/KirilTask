...
export const getDataLogic = createLogic({
  ...
  process({ state, action }, dispatch, done) {
    (async function() {
      const data = await fetch(`https://api.escuelajs.co/api/v1/categories/4/products?offset=${action.payload}&limit=${PAGINATION_LIMIT}`)
...
export const refreshDataLogic = createLogic({
  ...
  process({ state, action }, dispatch, done) {
    (async function() {
      const data = await fetch(`https://api.escuelajs.co/api/v1/categories/4/products?offset=0&limit=8`)
        .then(response => response.json())
